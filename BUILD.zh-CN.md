...
S20 移植是一个**四 DEX 应用**，且其主 DEX 依赖全部 163 个被补丁修改的文件。之前的公开构建流程只重建了主 DEX 的一部分、也不带次级 DEX，生成的 APK 可以安装、签名也正确，但无法注册 IMS。该流程及其 `smali_patch.diff` 已被删除——`patches/` 是 smali 改动的唯一来源，`smali_out/` 只是受审阅的快照，绝不是独立的构建输入。
