...
> 的单 DEX 流程并不完整，会构建出无法注册 IMS 的 APK。请改用
> **[BUILD.md](BUILD.md)** 中的分阶段流程：它会校验精确的原厂输入，依次应用
> `stock-to-desem5` 和 `desem5-to-desem81`，保留四个 DEX，并验证最终现代 IMS 服务实现。
...
本地 `classes2.dex`、Samsung framework 与 VSIM 输入的准备方式见
[BUILD.md](BUILD.md)。`build.sh` 仍只负责编译 Java bridge；完整 APK 由分阶段组装脚本生成。
