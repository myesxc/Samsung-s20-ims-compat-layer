...
(
    ...
    while true; do
        # Drop any stale node first. ims_sock_launch unlinks and re-binds, so a fresh node is
        # created on every restart and inherits /dev/socket's generic socket_device label. If a
        # stale node were still present, the relabel loop below could match it, break, and leave
        # the newly bound socket unlabelled — the denial would silently return after any restart.
        rm -f /dev/socket/imsd

        # Launch asynchronously so the socket can be relabelled before imsservice connects.
        # The helper execv()s imsd in place, so this pid stays valid and wait returns imsd's code.
