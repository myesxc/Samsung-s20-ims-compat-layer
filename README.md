...
```bash
source tools/env.sh
bash ./build.sh                 # Java bridge sources -> smali_out/
...
See [BUILD.md](BUILD.md) for the required local `classes2.dex`, Samsung-framework and VSIM
inputs. `build.sh` remains the Java bridge compiler; the staged assembly script builds the
complete APK.
